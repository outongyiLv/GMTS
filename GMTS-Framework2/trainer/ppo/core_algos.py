...
def compute_policy_loss(old_log_prob,
    ...
    pg_clipfrac = verl_F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask) #与mask乘 统计裁剪比例
    ...
    pg_losses = torch.where(advantages < 0, clip_pg_losses2, clip_pg_losses1) # A<0使用双重Dual-CLip, 否则使用 clip_pg_losses1
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode) # token-mean
