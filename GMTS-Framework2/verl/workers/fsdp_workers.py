...
class ActorRolloutRefWorker(Worker):
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp) # 这里分布式的load这个model到gpu上
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name='update_policy', logger=None) as timer:
                metrics = self.actor.update_policy(data=data) # 这里执行更新
    ...
    def generate_sequences(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(torch.cuda.current_device()) # 把所有prompts放到device上
    ...
    # loty-add: the function to calculate the entropy here.
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_entropy(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            output = self.actor.compute_entropy(data=data) # 获得entropy
            output = DataProto.from_dict(tensors={'old_entropys': output}, meta_info={'temperature': self.config.rollout.temperature})
            output = self.ulysses_sharding_manager.postprocess_data(output)
        ...
        log_gpu_memory_usage('After compute_entropy', logger=logger)
        return output
