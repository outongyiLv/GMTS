...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(self, micro_batch, temperature) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                # pad and slice the inputs if sp > 1 # False
    ...
    # loty-add:function to calculate the entropy
    ...
    def update_policy(self, data: DataProto):
        ...
        doing_entropy_clipping_type    = data.meta_info['doing_entropy_clipping_type']
        doing_entropy_clipping_percent = data.meta_info['doing_entropy_clipping_percent']
        temperature = data.meta_info['temperature']  # temperature must be in the data.meta_info to avoid slient error
            ...
            for batch_idx, data in enumerate(dataloader): 
                ...
                if ((doing_entropy_clipping_type=="ETS") or (doing_entropy_clipping_type=="GMTS")) :  
                    ...
                    if(doing_entropy_clipping_type=="ETS"):
                        all_entropy_now_flatten = all_entropy_now.flatten()  
                        non_zero_all_entropy_now_flatten = all_entropy_now_flatten[all_entropy_now_flatten!=0] 
                        sorted_elements, sorted_indices = torch.sort(non_zero_all_entropy_now_flatten) 
                    ...
                    elif(doing_entropy_clipping_type=="GMTS"):
                        ...
                        assert all_entropy_now.size()[0]%16==0 
                        ...
                        for i in range(0, number_question, 16):
                            ...
                            sorted_elements, sorted_indices = torch.sort(non_zero_all_entropy_now_flatten_adv_group) # sorting 
                ...
                # step4: after changing, seperating again
                if has_multi_modal_inputs:
                    self.gradient_accumulation = self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu
                    num_micro_batches = mini_batch.batch.batch_size[0] // self.config.ppo_micro_batch_size_per_gpu
                    micro_batches = data.select(select_keys, non_tensor_select_keys).chunk(num_micro_batches)
                ...
                for data in micro_batches:
                    ...
                    # all return: (micro-bsz, response_length)
                    ...
                    # compute policy loss  (-entropy trade)
